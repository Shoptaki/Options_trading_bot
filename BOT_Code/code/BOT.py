...
global client, client_thread, NY_TZ, event_queue, order_mgr, reconnect_time, dataStrike, totalTradeDay, signal_dict
...
def placeOrder(symbol, expiry=None, strike=None, right=None, action=None,
    ...
    try:
        ...
        logger.info("\nUPDATING TRADE COUNT FOR TODAY BEFORE UPDATE. CURRENT IS = {}".format(totalTradeDay))
        totalTradeDay = totalTradeDay + 1
        logger.info("\nUPDATING TRADE COUNT FOR TODAY AFTER UPDATE. CURRENT IS = {}".format(totalTradeDay))
...
def main_call(data):
    ...
    global IP, PORT, CLIENTID, SUB_ACCOUNT_ID, fetchValue, candleTime, stockListDict, stockList, dataInFile, EXPIRY, useAmount, MARKET_START_TIME, startTime, endTime, VWAP_ON_OFF, TRANSMIT, ORDER_EXPIRY_TIMER, USE_TIMER_IN_ORDER, CALL_DELTA_CHECK, PUT_DELTA_CHECK, VOLUME_CHECK, ATR_CHECKS, ACTIVE_VOLUME, MAX_CONTRACT_AMOUNT, ATR_VALUE, SHARE_VOLUME, BODY, perDayTrades, USE_DIFF_EXPIRY_INDEX, SPY_QQQ_EXPIRY, PROFIT_INCREMENT, DISTANCE_BETWEEN_TRADE, EXPIRY, tradeExpiry
    global trade_time_dict, totalTradeDay, signal_dict
    ...
    BODY = data["BODY"]
    perDayTrades = data["perDayTrades"]
    ...
    while True:
        ...
        #logger.info("int(totalTradeDay) = {} and int(perDayTrades) = {}".format(int(totalTradeDay), int(perDayTrades)))
        if int(totalTradeDay) > int(perDayTrades):
            logger.info("\n\n************* TOTAL TRADES = {} ARE DONE FOR TODAY. EXIT SYSTEM****************\n\n".format())
            sys.exit(0)
